...
export default function DeanView({ user }: Props) {
  const [activeTab, setActiveTab] = useState<"gate" | "rooms" | "attendance">("attendance");
  ...
  const [qrTokenData, setQrTokenData] = useState<any>(null);
  ...
  const fetchGateData = async () => {
    ...
    try {
      const [passesRes, registryRes] = await Promise.all([
        dispatchGateway(GATEWAY_OPCODES.FETCH_DEAN_DASHBOARD),
      ...
      if (passesRes.ok && passesRes.data) {
        ...
        if (Array.isArray(passesRes.data.sessions)) {
          setSessions(passesRes.data.sessions);
          const active = passesRes.data.sessions.find((s: any) => s.status === "ACTIVE");
  ...
  const handleCreateSession = async () => {
    if (!newEventName.trim()) {
      setActionAlert({ type: "error", message: "Event Name is required." });
      return;
    }
    const res = await dispatchGateway(GATEWAY_OPCODES.CREATE_ATTENDANCE_SESSION, { eventName: newEventName.trim() });
    if (res.ok) {
      setActiveSession(res.data?.session);
      setNewEventName("");
      setActionAlert({ type: "success", message: "Session created successfully." });
    } else {
      setActionAlert({ type: "error", message: res.message || "Failed to create session." });
    }
  };

  const handleToggleLock = async (lock: boolean) => {
    if (!activeSession) return;
    const res = await dispatchGateway(GATEWAY_OPCODES.TOGGLE_LOGIN_LOCK, { sessionId: activeSession._id, lock });
    if (res.ok) {
      setActiveSession(res.data?.session);
    } else {
      setActionAlert({ type: "error", message: res.message || "Failed to toggle lock." });
    }
  };
  ...
  const handleGenerateQR = async () => {
    ...
    if (!activeSession.loginLocked) {
      setActionAlert({ type: "error", message: "Please lock login/logout before generating QR." });
      return;
    }
    setQrLoading(true);
  ...
  const handleEndSession = async () => {
    if (!activeSession) return;
    if (!confirm("Are you sure you want to end this session? This will generate the Excel and send it via email.")) return;
    const res = await dispatchGateway(GATEWAY_OPCODES.END_ATTENDANCE_SESSION, { sessionId: activeSession._id });
    if (res.ok) {
      ...
      fetchGateData();
      setActionAlert({ type: "success", message: `Session ended. Excel generated for ${res.data?.totalRecords} attendees.` });
    } else {
      setActionAlert({ type: "error", message: res.message || "Failed to end session." });
    }
  };

  useEffect(() => {
    // Only auto-refresh if we already generated the FIRST token manually
    if (activeSession && activeSession.loginLocked && activeTab === "attendance" && qrDataUrl) {
      qrIntervalRef.current = setInterval(() => {
        handleGenerateQR();
      }, 5000);
    } else if (qrIntervalRef.current) {
      clearInterval(qrIntervalRef.current);
    }
    return () => {
      if (qrIntervalRef.current) clearInterval(qrIntervalRef.current);
    };
  }, [activeSession?.loginLocked, activeTab, qrDataUrl]);
